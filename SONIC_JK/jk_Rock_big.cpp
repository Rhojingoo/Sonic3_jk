...
namespace jk
{
	...
	void Rock_big::Initialize()
	{
		...
		mAnimator->CreateAnimation(L"Big_Rock", mImage, Vector2(276, 578), Vector2(48, 80), Vector2(0, 0), 1, 1, 1, Vector2::Zero, 0.1f);
	...
	void Rock_big::Update()
	{
		...
		if (ROCK_TR && ROCK_rb && mGroundImage)
		{
			...
			COLORREF RING_Color = static_cast<int>(mGroundImage->GetPixel(static_cast<int>(ROCK_ps.x), static_cast<int>(ROCK_ps.y) + 160));
